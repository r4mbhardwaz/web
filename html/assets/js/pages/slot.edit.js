...
window.submitNewData = function(ev) {
    ...
    http.post(`/api/slot/${slotId}/add-data`, {
        value: value,
        synonyms: synonyms
...
window.slotValueDelete = function(ev) {
    ...
    http.get(`/api/slot/${slotId}/delete/${target.dataset.slotdataid}`)
        .then(JSON.parse)
        .then(d => {
...
window.updateSlotValue = function(newValue, element, oldValue="") {
    ...
    http.post(`/api/slot/${slotId}/${itemId}/change`, { value: newValue }).then(JSON.parse).then(d => {
        if (!d.success) {
...
window.updateSlotSynonyms = function(newValue, element, oldValue="") {
    const slotId = qry("[data-slotid]").get(0).dataset.slotid;
    const itemId = element.dataset.itemid;
    http.post(`/api/slot/${slotId}/${itemId}/change`, { synonyms: newValue }).then(JSON.parse).then(d => {
        if (!d.success) {
...
qry("[data-emptyslot]").click(ev => {
    ...
    http.post(`/api/slot/${slotId}/empty`)
    .then(JSON.parse)
    .then(d => {
        if (d.success) {
...
window.showCSVColumnSelector = function(data, headers, result) {
    ...
    allCells.click(ev => {
        ...
        window._papaConfig.complete = function(myData, x) {
            ...
            http.post(`/api/slot/${slotId}/import`, {
                values: relevantData
            })
            .then(JSON.parse)
            .then(d => {
...
infiniteScroll(_ => {
    ...
    return new Promise(function(resolve, reject) {
        ...
        http.post(`/api/slot/${slotId}/load-data`, {
