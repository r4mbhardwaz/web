...
window.searchIcon = function(searchTerm) {
    http.get(`/api/search-icon?search=${searchTerm}`).then(JSON.parse).then(d => {
...
window.searchRepos = function(value) {
    http.get(`/api/git/repos/${value}`).then(JSON.parse).then(d => {
        let code = ``;
...
document.querySelector("#create-skill").addEventListener("click", ev => {
    ...
    http.post(`/api/skill/new`, skillObject)
    .then(JSON.parse)
    .then(d => {
        if (d.success) {
